...
At its core, Tropical is just [four files](#core-files).
...
**By default, components used in pages are only prerendered.** Any client-side behaviour in your component (e.g. event handlers, timers, `useEffect` hooks or lifecycle methods) will not work in the browser until you [`hydrate`](https://reactjs.org/docs/react-dom.html#hydrate) the prerendered component.

You can do this with the provided `hydration_helpers.js` or DIY manually in `entry.client.js`.

**Hydrating React is totally optional!** You can add behaviour to your prerendered HTML using vanilla JS, other libraries, or by manually mounting client-only React components.

## Core files

These 4 small files in the `app` directory are the heart of Tropical. They're heavily commented, so I encourage you to read them, understand what's happening and make changes to suit your needs!

- `build.js`
- `entry.client.js`
- `entry.prerender.js`
- `hydration_helpers.js`
...
- **Rehydrate Fela styles:** If you use Fela in your client bundle (including in any components), you need to [hydrate](https://fela.js.org/docs/api/fela-dom/rehydrate.html) fela-dom's cache from prerendered styles.
- **Import and hydrate prerendered components:** See `hydration_helpers.js`
...
A [higher-order component](https://reactjs.org/docs/higher-order-components.html) for prerendering components in your pages that will later be hydrated with the same props on the client.

```js
asIsland(componentName, Component, options)
```

- `componentName`: the name of the component as passed to `hydrateIslands` *(string, required)*
...
Used by the browser JS bundle to [hydrate](https://reactjs.org/docs/react-dom.html#hydrate) any components that were prerendered using the `asIsland` higher-order component.

```js
hydrateIslands(components)
```

- `components`: an object containing the components to be hydrated, keyed by the `componentName` values used in `asIsland` *(object, required)*

#### Example usage of hydration helpers
...
Finally, ensure the original component is imported into `entry.client.js` and passed to `hydrateIslands`:

```js
import WelcomeBanner from '../components/welcome_banner/welcome_banner'
hydrateIslands({ WelcomeBanner })
```
...
```html
<div
  data-tropical-hydrate-as='WelcomeBanner'
...
This element will be detected by `hydrateIslands` and your component will be hydrated with its original props.
